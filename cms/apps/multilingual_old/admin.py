...
class MultilingualForm(forms.ModelForm):
    def clean(self):
        cleaned_data = super(MultilingualForm, self).clean()

        # Check to see if we have an object, should be a MultilingualObject
        if self.obj:

            # Check to see if a language object exists
            duplicate_check = self.obj.parent.language_model.objects.filter(
            ...
            if duplicate_check.count() == 1:
                ...
                # If the duplicate found is the current object, ignore errors
                if duplicate_check[0] != self.obj:
                    self.add_error('language', '')
                    self.add_error('version', 'A version for this language already exists, try increasing the version number')
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        for field in language_class._meta.fields + language_class._meta.many_to_many:
            ...
            if request.resolver_match.url_name in [
                ...
                if field.name == "default_language":
                    continue
        ...
        if six.PY2:
            ContentForm = type(six.binary_type("{}Form").format(self.__class__.__name__), (MultilingualForm,),
                               form_attrs)
        else:
            ContentForm = type(six.text_type("{}Form".format(self.__class__.__name__)), (MultilingualForm,),
                               form_attrs)
